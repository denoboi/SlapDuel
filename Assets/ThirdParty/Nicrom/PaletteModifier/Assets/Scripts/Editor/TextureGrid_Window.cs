...
namespace Nicrom.PM {
    public class TextureGrid_Window : EditorWindow{
        ...
        private void DrawBackgroundGrids()
        {
            ...
            DrawBackgroundGrid((tg.texAtlas.width / 16) * scale, 0.2f, Color.gray);
            DrawBackgroundGrid((tg.texAtlas.width / 4) * scale, 0.4f, Color.gray);
        }
        ...
        {
            Vector2Int sizeOffset = new Vector2Int(0, 0);
            int zoomAmount = tg.zoomSpeed;

            if (tg.sizeOffset.x > 0 && tg.sizeOffset.y > 0 && tg.texAtlas.width > 512 && tg.texAtlas.height > 512)
                zoomAmount = Mathf.RoundToInt(tg.zoomSpeed * ((tg.sizeOffset.x + tg.texAtlas.width) / tg.texAtlas.width));

            sizeOffset.x = tg.sizeOffset.x + Mathf.RoundToInt(-delta.y * zoomAmount);
            sizeOffset.y = Mathf.RoundToInt((sizeOffset.x * tg.texAtlas.height) / tg.texAtlas.width);

            if (tg.texAtlas.width + sizeOffset.x > 64 && tg.texAtlas.height + sizeOffset.y > 64)
            {
                tg.originOffset = TG_Utils.OffsetOriginOnCanvasZoom(tg, delta.y * zoomAmount);

                tg.sizeOffset.x += Mathf.RoundToInt(-delta.y * zoomAmount);
                tg.sizeOffset.y = Mathf.RoundToInt((sizeOffset.x * tg.texAtlas.height) / tg.texAtlas.width);
