...
internal class ToolbarSceneSelection : BaseToolbarElement {
				...
				foreach (var scenePath in scenesPath) {
					if ((scenePath) == scenesPopupDisplay[selectedSceneIndex].path) {
						...
						if(scenePath.Contains("Level"))
                        {
                            ...
                            {
								if(scenePath.Contains(LevelData.Levels[i].LoadLevelID))
                                {
									PlayerPrefs.SetInt(PlayerPrefKeys.LastLevel, i);
									break;
                                }
							}
						}
	...
	void RefreshScenesList() {
		...
		string[] sceneGuids = AssetDatabase.FindAssets("t:scene", new string[] { "Assets", "Packages" });
