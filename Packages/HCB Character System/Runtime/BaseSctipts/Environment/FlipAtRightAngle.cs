...
namespace CMF
{
	...
	public class FlipAtRightAngle : MonoBehaviour {
		...
		{
			if(col.GetComponent<ICharacterController>() == null)
				return;

			SwitchDirection(tr.forward, col.GetComponent<ICharacterController>());
		}

		void SwitchDirection(Vector3 _newUpDirection, ICharacterController _controller)
		{
